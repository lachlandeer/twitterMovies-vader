...
rule runChicagoDaily:
    input:
        dataStats = expand(config["out_chicago_counts"] + "minute-0.05/" + "{iFolder}.csv", \
                            iFolder = CHICAGODATA),
        dataCounts = expand(config["out_chicago_stats"] + "minute-0.05/" + "{iFolder}.csv", \
                            iFolder = CHICAGODATA)
...
rule chicagoDaily:
    ...
    output:
        outCounts = config["out_chicago_counts"] + "minute-0.05/" + "{iFolder}.csv",
        outStats  = config["out_chicago_stats"]  + "minute-0.05/" + "{iFolder}.csv"
    log: config["out_log"] + "minute-0.05/" + str("{iFolder}") + "_" + \
                         "daily.txt"
    shell:
        "{RUN_PYSPARK} \
...
rule gnipDaily:
    ...
    params:
        ...
        dataPath   = config["data_mount"]
    output:
        outCounts = config["out_gnip_counts"] + "minute-0.05/" + "{iChunk}.csv",
        outStats  = config["out_gnip_stats"]  + "minute-0.05/" + "{iChunk}.csv"
    log: config["out_log"] + "minute-0.05/" + "{iChunk}_vaderDaily.txt"
    shell:
        "{RUN_PYSPARK} \
            --py-files {input.library} \
            {input.script} --dataPath {params.dataPath} \
